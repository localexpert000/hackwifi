An idea from Sandman: Include "Evil Twin" attack in Hackwifi.
...
Below are all of the requirements/components that Hackwifi would need for this feature.
...
I think MDK is a better tool for this job, but Hackwifi already requires the `aircrack` suite, so we should support both.
