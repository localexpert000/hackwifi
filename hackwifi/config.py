...
class Configuration(object):
    ''' Stores configuration variables and functions for Hackwifi. '''
    version = '2.2.5'
    ...
    def parse_wpa_args(cls, args):
        ...
        if args.wordlist:
            if not os.path.exists(args.wordlist):
                cls.wordlist = None
                Color.pl('{+} {C}option:{O} wordlist {R}%s{O} was not found, hackwifi will NOT attempt to crack handshakes' % args.wordlist)
                ...
                cls.wordlist = None
                Color.pl('{+} {C}option:{O} wordlist {R}%s{O} is a directory, not a file. Hackwifi will NOT attempt to crack handshakes' % args.wordlist)
